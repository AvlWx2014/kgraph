...
class MutableUndirectedGraph<V : Any, E : Any> : UndirectedGraph<V, E>(), MutableGraph<V, E> {
    ...
    override fun addEdge(u: Vertex<V>, v: Vertex<V>, value: E): Boolean = addEdge(UndirectedEdge(u, v, value))
    override fun addEdge(edge: Edge<V, E>): Boolean {
        require(edge is UndirectedEdge) {
        ...
        v.addNeighbor(u, edge)
        u.addPredecessor(v, edge)
    ...
    override fun removeEdge(edge: Edge<V, E>): Boolean {
        if (edge !is UndirectedEdge || edge !in edgeMap) {
            return false
        }
